...
**H2.1**: 선형 앙상블이 곱/최대 앙상블 대비 **최저 ECE** 달성. (⚠️ 실험 결과: calibration leakage 수정 후 ECE ≈ 0.19, 기각됨)
...
**H4.4**: 비용 민감성이 **탐지기 품질**과 상호작용: (⚠️ 실험 결과: ECE<0.05 달성 못함, 전제 붕괴)
- 잘 보정된 탐지기 (ECE < 0.05): 비용 비율이 직접 임계값으로 변환 → 달성 불가 (leakage 수정 후 ECE ≈ 0.19)
